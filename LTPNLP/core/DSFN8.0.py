        ...
        if item1.dependency == "SBV" and item1.head_word.postag == "v":
            ...
            if item2.dependency == "VOB" and item2.head_word.postag == "v":
                ...
                if (len(predDict) == 1):
                    ...
                    # print(item1.lemma + " " +item2.lemma+" "+wordAfterOBJ.lemma)
                    """
                    考虑“1961年中华人民共和国国务院公布故宫为全国重点文物保护单位”
                    """
                    if wordAfterOBJ!=None and wordAfterOBJ.postag == 'v' and wordAfterOBJ.dependency == 'COO' and wordAfterOBJ.head_word.ID == pred1.ID:
                        ...
                        if hasPOB == False:
                            for i in range(pred.ID + 1, len(sentence.words) + 1):
                                ...
                                if item.dependency == "CMP" and item.head_word.ID == pred.ID:
                                    ...
                                    objectDict[object.ID] = object
                                    for word in sentence.words:
                                        if word.head_word != None and word.dependency == "VOB" and word.head_word.ID == object.ID:
                                            objectDict[word.ID] = word
                                    flagLoop = True
                                    while flagLoop == True:
                                        len1 = len(objectDict)
...
if __name__ == '__main__':
    ...
    new_sentence = []
    sentence_list = [
        "溥心畲书法作品最多的是行楷书作，而临习金文之作，殊为难得，此为其精心之品"]
    # all_entity = ['小明','小王','豪宅','厦门大学']
    count = 0
    allTripesForGraph = []
    noBlankInSentence = []
