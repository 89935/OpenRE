                            ...
                            if item.head_word != None:
                                ...
                                if item.head_word.ID in SUBList and (item.dependency == "ATT"):
                                    if item.dependency == "ATT" and item.postag == "v":
                                        if self.get_entity_num_between(item, item1, sentence) > 0:
                                            continue
                                        else:
                                            SUBATT = item
                        ...
                        for item in sentence.words:
                            if item.head_word != None:
                                ...
                                if item.head_word.ID in OBJList and (item.dependency == "ADV" or item.dependency == "ATT" or item.dependency == "VOB"
                                    ...
                                    if item.dependency == "ATT" and item.postag == "v":
                                        if self.get_entity_num_between(item,item2,sentence)>0:
                                            continue
                                        else:
                                            OBJATT = item
                    ...
                    while flag == True:
                        ...
                        for item in sentence.words:
                            if item.head_word != None:
                                ...
                                if item.head_word.ID in SUBList and (item.dependency == "ATT"):
                                    if item.dependency == "ATT" and item.postag == "v":
                                        if self.get_entity_num_between(item,sentence.get_word_by_id(sub),sentence)>0:
                                            continue
                                        else:
                                            SUBATT = item
...
if __name__ == '__main__':
    ...
    # sentence_list = ["1912年，溥伟将恭王府所藏文物（除字画外），全部卖给日本山中商会的山中定次郎"]
    # all_entity = ["德国","默克尔","美国","奥巴马"]
    ...
    with open('outputTripes\\天坛-带有所有修饰的结果-9.4' + now+".json", 'w',
