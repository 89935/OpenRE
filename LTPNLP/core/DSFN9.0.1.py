...
class DSFN:
    ...
    def __init__(self, model_dir=default_model_dir, all_entity=all_entity):
        ...
        segmentor_flag2 = self.segmentor2.load(os.path.join(default_model_dir, 'cws.model'))
        # 词性标注模型
    ...
    def segment2(self, sentence, entity_postag=dict()):
        words = self.segmentor2.segment(sentence)
        lemmas = []
        for lemma in words:
            lemmas.append(lemma)
        return lemmas
    ...
    def dsfnStart2(self, rawSentence, entity1, entity2,all_entity):
        nounRelatedWithPosition = ['主席','总理','教授','校长']
        resultList = []
        lemmas = dsfn.segment2(rawSentence)
        words = dsfn.postag(lemmas)
        words_netag = dsfn.netag(words)
        sentence = dsfn.parse(words_netag)
        # print(sentence.to_string())
    ...
    def getSPO2(self,sentence):
        all_result = []
        raw_sentence = []
        RawSentence = sentence
        lemmas = self.segment2(sentence)
        words = self.postag(lemmas)
        words_netag = self.netag(words)
        sentence = self.parse(words_netag)
        # print(sentence.to_string())
...
if __name__ == '__main__':
    ...
    f = open('D:\python-file\北京市旅游知识图谱\\verb-entity\\bj_travel\\' + "5A_恭王府.txt"
             , 'r', encoding='utf-8')
    file = f.read()
    #    print(file)
    ...
    new_sentence = []
    # sentence_list = ["乾隆皇帝为孝敬其母孝圣皇后动用448万两白银在这里改建为清漪园"]
    count = 0
    allTripesForGraph = []
    noBlankInSentence = []
        ...
        for sentenceConstraint1 in sentenceListConstraint1:
            ...
            for index1 in range(0, len(all_entity)):
                ...
                for index2 in range(index1 + 1, len(all_entity)):
                    ...
                    tripes = dsfn.dsfnStart2(sentenceConstraint1, all_entity[index1], all_entity[index2], all_entity)
                    # print(all_entity[index1])
                    # print(all_entity[index2])
                    # print("dsfn产生的结果:"+str(tripes))
                    if tripes != None and len(tripes) != 0:
            ...
            allTripes, raw_sentence = dsfn.getSPO2(sentenceConstraint1)
            PostTripes = dsfn.PostProcessSPO(raw_sentence, allTripes, all_entity)
            if PostTripes != None and len(PostTripes) != 0:
                for tripe in PostTripes:
                    final_result_tripe.append(tripe)
