# -*- coding: utf-8 -*-
"""
Created on Sun Jan  5 13:14:05 2020
...
class ExtractByDSNF:
    ...
    def __init__(self,origin_sentence,sentence,entity1,entity2,file_path,num,nlp):
    ...
    def build_triple(self,entity1,entity2,relation):
        ...
        words = self.nlp.postag([entity1_str,relation_str,entity2_str])
        triple['知识'] = [entity1_str+'_'+words[0].postag,relation_str+'_'+words[1].postag,entity2_str+'_'+words[2].postag]
        knowledge_dict['知识'] = [entity1_str+'_'+words[0].postag,relation_str+'_'+words[1].postag,entity2_str+'_'+words[2].postag]
        onlyAppendKnowledgeToJson().append(self.file_path[:11]+'only_knowledge_'+self.file_path[11:],knowledge_dict)
    ...
    def SBV_CMP_POB(self,entity1,entity2):
        """IVC(Intransitive Verb Construction)[DSNF4]
            不及物动词结构的一种形式，例如：“奥巴马毕业于哈佛大学” ---> "奥巴马 毕业 于 哈佛 大学"
            经过命名实体后，合并分词结构将是"奥巴马 毕业 于 哈佛大学"
            entity1 --->"奥巴马"
            entity2 --->"哈佛大学"
            Args：
        ...
        #该实例对应为“哈佛大学”
        #但当“哈佛”的命名实体识别为“S-Ni”时，命名实体识别后将是“哈佛 大学”，因为：哈佛<-[ATT]-大学
        #得到的将是["奥巴马"，“毕业于”，“哈佛大学”]
    ...
    def determine_relation_SVB(self,entity1,entity2,ent1,ent2):
        ...
        if coo_flag:
            ...
            #[DSNF7]
            #如果实体2所依存的词，与实体1所依存的词构成COO，那么特征关系词选择实体2所依存的词
            #习近平 视察 并 访问 厦门
...
#----------------------------------------------------------------------------
    ...
    def coordinate(self,entity1,entity2):
        ...
        [DSNF3|DSNF5|DSNF6]
        并列实体
        当实体存在COO时，如果实体1和实体2并列，实体2与实体3构成三元组，则实体1和实体2也会构成三元组
        ...
        #依据“李克强”（entity1） 和 “美国”（entity2），抽取三元组（李克强，访问，美国）
        #如果存在并列依存，只会有entity1(ent1)<-[ATT]-entity2(ent2)
        #entity1与entity2不构成主宾entity1(ent1)<-[ADV]- temp-[VOB]-> entity3
    ...
    def SBVorFOB_POB_VOB(self,entity1,entity2,entity_coo= None, entity_flag = ''):
        """[DSNF3]
            实体1依存于V（SBV或前置宾语），实体2依存于一个介词（POB），且介词依存于V（ADV），一个名词
            （将作为特征关系词）依存于V(VOB)
            习近平 对 埃及 进行 国事访问
            Args :
    ...
    def determine_relation_SVP(self, entity1, entity2, ent1, ent2):
        ...
        if ent_2 != entity2 and abs(ent_2.ID - entity2.ID)==1:
            entity2_list.append(ent_2)
            if ent_2.dependency =='ATT' and abs(ent_2.head - entity2.ID)<=3:
                entity2_list.append(ent_2.head)
...
#-------------------------------------------------------------------------- 
    def E_NN_E(self,entity1,entity2):
        """DSNF1
        如果两个实体紧紧地靠在一起，第一个实体是第二个实体的ATT，两个实体之间的词性为NNT（职位名词）
        Args：
            entity1：WordUnit，原实体1
            entity2：WordUnit，原实体2
        Returns：
        ...
        if entity1.dependency == 'ATT' and entity1.head_word.dependency =='ATT' and entity1.head_word.head == entity2.ID:
            ...
            #如果temp前还有其他名词修饰器修饰
            #美国 前任 总统 奥巴马 | 中国 现任 主席 习近平
            #“美国” <--[ATT]-- 主席  主席<-[ATT]--习近平  前任<---主席
        ...
        if self.sentence.words[entity1.ID].lemma == '的':
            if(entity1.head == entity2.head or entity1.head_word.head == entity2.ID
               and 'n' in entity1.head_word.postag and entity1.ID<entity1.head):
                if entity2.postag == 'nh' and abs(entity1.ID-entity2.ID)<4:
        ...
        if temp:
            ...
            while i<=entity2.ID-2:
                ...
                if word.lemma =='的' and word.dependency == 'RAD' and word.head_word.head == temp.ID:
                    ...
                    self.build_triple(entity1_list, entity2_list, relation_list)#田蕴哲认为所抽出的三元组是这样的(朱崇实，厦门大学的，校长)
                    ok = True
                    break
                i +=1
        return ok