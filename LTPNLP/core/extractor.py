# -*- coding: utf-8 -*-
"""
Created on Mon Jan  6 10:05:36 2020
...
from LTPNLP.bean.entity_pair import EntityPair
from LTPNLP.core.extract_by_dsnf import ExtractByDSNF
...
class Extractor:
    ...
    """
    entities=[]#存储该句子中可能实体
    entity_pairs = [] #存储句子中（满足一定条件）的可能实体对
    
    def extract(self,origin_sentence,sentence,file_path,num,nlp):
        """
        Args:
        ...
        for entity_pair in self.entity_pairs:
            ...
            extract_dsnf = ExtractByDSNF(origin_sentence,sentence,entity1,entity2,file_path,num,nlp)
