...
Remember that skipgram tries to predict each context word from its target word,
and so the task becomes to predict 'the' and 'cs224n' from 'first', 'lot' and 'fun' from 'of' and so on.
...
def skipgram():
    ...
    with tf.variable_scope('word2vec') as scope:
        ...
        # This objective is maximized when the model assigns high probabilities to the real words, and low
        # probabilities to noise words
...
visualize_embedding(final_embeddings,reverse_dictionary)
