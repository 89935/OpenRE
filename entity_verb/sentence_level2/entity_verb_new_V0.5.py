...
class entity_verb_new:
    ...
    def __init__(self):
        ...
        file = f.read()
        self.location_entity = json.loads(file)['地点--500']  # 3、修改为json文件中“地点”实体列表所对应的key
        f.close()
                    ...
                    while location<len(sentence):
                        ...
                        if location ==-1:
                            ...
                            for word,postag in zip(newWordList,newPosTagList):
                                charCount += len(word)
                                if word == entity and location + len(word) == charCount:
                                    entityPos = postag
                                    if entityPos == 'ns':
                                        count = 0
                                elif count == 0 or count == 1:
                                    if postag == 'v' or postag == 'p' or postag == 'wp' or word == '，' or word == '、':
                                        break

                                    if postag == 'nd' and ('东' in word or '南' in word or '西' in word or '北' in word or '附近' in word or '周围' in word
                                                           or '两侧' in word):
                                        newEntity += word
                                        print("LTP已修改entity")
                                        print(entity)
                                        ...
                                        count += 1
                            entity_verb_dict[str(index)+'#'+newEntity + '_A' + entityPos] = location
                            location+=len(entity)
                        ...
                        if location ==-1:
                            ...
                            if entity in self.tempNoun:
                                ...
                                for word, postag in zip(newWordList, newPosTagList):
                                    charCount += len(word)
                                    if word == entity and location + len(word) == charCount:
                                        if word in self.location_entity:
                                            count = 0
                                    elif count == 0 or count == 1:
                                        if postag == 'v' or postag == 'p' or postag == 'wp' or word == '，' or word == '、':
                                            break
                                        if postag == 'nd' and ('东' in word or '南' in word or '西' in word or '北' in word or '附近' in word or '周围' in word
                                                           or '两侧' in word ):
                                            newEntity +=word
                                            print("XLORE已修改entity")
                                            print(entity)
                                            break
                                        else:
                                            count += 1
                                entity_verb_dict[str(index)+'#'+newEntity + '_' + 'ns'] = location
                            location+=len(entity)
...
if __name__ == "__main__":
    ...
    for file_name in file_list:
        ...
        # sentence_list = ['位于圜丘东，座北朝南，院门南开，主要建筑有神库、神厨、井亭，是圜丘冬至祭天大典之前制作圜丘坛各种祭品的场所"']
        noBlankSentenceList = []
        for sentence in sentence_list:
        ...
        with open('sentence_level2_result\\V0.5\\' + file_name[:-3]+"json", 'w',
