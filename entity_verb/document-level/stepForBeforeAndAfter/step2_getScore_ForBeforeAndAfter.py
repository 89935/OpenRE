...
import json
import math
import step1_getIG_ForBeforeAndAfter
import step4_filterTriples_ForBeforeAndAfter
def getScore(types,rel):
    ...
    """
    scoreDict = dict()
    probOfTGivenRelDict = step1_getIG_ForBeforeAndAfter.givenRelGetProbOfType(types, rel)  # 计算P(t|rel)，关系为rel的条件下，实体对类型为t的概率
    relAndTypeDict = step1_getIG_ForBeforeAndAfter.getRelTripes(types, rel)  # 计算C(rel,t):rel和t共线的次数
    print("p(t|rel)")
...
def sortedByScore(types):
    ...
    :return:
    """
    allCandidateRelSet = step1_getIG_ForBeforeAndAfter.getAllCandidateRel(types)
    ...
    for type in types:
        name += "(" + type + ")+"
    step4_filterTriples_ForBeforeAndAfter.writeJsonFile(sortedScoreByTypeDict, types, name + "Score.json")
