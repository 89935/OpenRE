import step1_getIG,step2_getScore,step4_filterTriples
import json
def filterCandidateRel(type_list,N,K):
    ...
    """
    IGCandidateRelationWords = step1_getIG.sortedByIG(type_list)  # 候选关系词按照IG(rel)值降序排列的结果
    ...
    for rel in relAndIG:
        IGList.append(rel[0])
    scoreCandidateRelationWords = step2_getScore.sortedByScore(type_list)  # 候选关系词按照score(rel,t)值降序排序结果
    ...
    for type in type_list:
        name += "(" + type + ")+"
    step4_filterTriples.writeJsonFile(relationWords, type_list, name + "RelationWords.json")
    step4_filterTriples.writeJsonFile(relationWordsAndScore, type_list, name + "RelationWordsAndScore.json")
...
if __name__ == "__main__":
    ...
    for type in type_list:
        name += "(" + type + ")+"
    step4_filterTriples.writeJsonFile(relationWords, name + "RelationWords.json")
    step4_filterTriples.writeJsonFile(relationWordsAndScore, name + "RelationWordsAndScore.json")